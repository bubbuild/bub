...
每个已注册钩子会列出贡献它的插件。该命令展示发现到的实现顺序，不等价于精确的 firstresult 胜出者；运行时优先级见[钩子参考](/zh-cn/docs/reference/hooks/#iteration-order)。一个覆盖了 `build_prompt` 与 `run_model_stream` 的 `EchoPlugin`，预期输出形如：
