...
Bub 从当前 Python 环境加载入口点，因此可编辑安装能在不重新安装的情况下吃到代码变更。同样的模式见[首个插件](/zh-cn/docs/getting-started/first-plugin/)。
...
每个已注册钩子会列出贡献它的插件。一个覆盖了 `build_prompt` 与 `run_model` 的 `EchoPlugin`，预期输出形如：
