...
底部工具栏通过 `speed:42.1 token/s` 显示最近一次模型调用的平均速度：模型返回的输出 token 数除以调用耗时，
包含请求等待时间，不包含工具执行时间。每次模型调用完成后更新，并在两轮对话之间保留；未返回用量时显示 `speed:- token/s`。
...
bub run ",echo hello"   # 找不到的命令会回落到 shell
```
