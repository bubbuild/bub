...
Bub 按以下优先级（从高到低）从三处读取配置：

1. 进程环境（`BUB_*` 变量，以及少量提供商变量如 `OPENROUTER_API_KEY`）。
2. 工作目录下的 `.env` 文件。
...
`bub onboard` 引导你完成最小配置 —— 模型、API key、可选的 channel 凭据 —— 并将结果写入 `~/.bub/config.yml`。

```bash
uv run bub onboard
```

文件默认位于 `~/.bub/config.yml`。设置 `BUB_HOME` 可整体迁移 `~/.bub/` 目录（包括配置、历史、插件项目）。
...
| `BUB_HOME` | Bub 的数据目录（配置、历史、插件项目）。默认 `~/.bub`。 |
...
预期输出是每条出站一行，带 `[<channel>:<chat_id>]` 前缀。
