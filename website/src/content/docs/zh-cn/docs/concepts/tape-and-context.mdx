...
生命周期结果以 `spill.write`、`spill.archive`、`spill.reset` 和 `sidecar.merge` event 记录在主 tape。它们会在任何 context selector 运行前被排除，因此可用于运维和审计，同时不会改变模型消息或 prompt cache 前缀。sidecar 持久化失败会被记录，但不会阻止主 tape merge 或 reset。
