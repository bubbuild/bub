...
builtin spill 插件通过 `provide_tape_sidecars` 挂载 `SpillStore`，并注册 `spill.read`。工具结果处理完全由插件所有，不向核心层暴露 spill 专用类型。较大的结果存放在名为 `<session-tape>__spill` 的 sibling tape 中。sidecar 与 session tape 使用同一个 `TapeStore`，因此现有存储插件不需要实现 spill 专用接口。Bub 依次写入 UTF-8 安全的 chunk，最后写入 manifest；manifest 是该结果已完整存储的提交标记。
