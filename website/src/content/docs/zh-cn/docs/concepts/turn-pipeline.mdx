...
流式过程中，每个 `error` 事件都会通过 `on_error(stage="run_model", ...)` 转发。绑定的 `ChannelRouter` 也可在 turn 结束前包装该流，将增量 chunk 推送到 channel。
...
- `provide_tape_sidecar` — 挂载在当前朴素 tape store 之上的 builtin `spill` 与 `forkmerge` sidecar。
