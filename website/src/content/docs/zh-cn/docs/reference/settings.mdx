...
2. **环境变量**(`BUB_*`)。
3. **`~/.bub/config.yml`** 中的字段,由 `bub.configure.load` 加载。
4. **字段默认值**,在 `Settings` 子类中声明。
...
`~/.bub/` 同时是 `bub.home` 的取值,由 `BUB_HOME` 环境变量控制(`src/bub/__init__.py`)。

## Framework —— 运行时路径

| 环境变量 | 默认值 | YAML 字段 | 读取方 | 描述 |
| --- | --- | --- | --- | --- |
| `BUB_HOME` | `~/.bub` | — | `bub.home` (`bub/__init__.py`) | 配置、tape store 与托管插件项目的根目录。 |
| `BUB_PROJECT` | `~/.bub/bub-project` | — | `bub install` / `uninstall` / `update` 的 `--project` 选项 | 插件项目目录;首次 `install` 时通过 `uv init` 创建。 |
