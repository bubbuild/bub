...
| `build_prompt` | firstresult | `(message: Envelope, session_id: str, state: State) -> str \| list[dict]` | prompt or content parts | `BubFramework.process_inbound` | 全部实现都返回 `None` 时回退到 `content_of(message)`。 |
...
| `save_state` | broadcast | `(session_id, state, message, model_output) -> None` | none | `process_inbound` finally block | 即使 turn 失败也会执行。 |
...
| `provide_channels` | sync-only consumer (deduped) | `(message_handler: MessageHandler) -> list[Channel]` | channels | `BubFramework.get_channels` (`call_many_sync`) | 按 `Channel.name` 去重,先注册的实现胜出。 |
...
:::caution
为同步钩子写 `async def` 实现会让它静默失效。请用 `bub hooks` 确认实际注册情况。
:::
