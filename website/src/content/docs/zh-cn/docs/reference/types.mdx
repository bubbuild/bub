---
title: 核心契约
description: Bub 中由 turn、model、tape、stream、hook 与 channel 子系统分别拥有的契约。
---
...
### Tape event 与 record — `bub.tape`

```python
from cloudevents.core.v1.event import CloudEvent
from bub.tape import TapeRecord, bub_event

event: CloudEvent = bub_event("diagnostic", {"status": "ok"})
record = TapeRecord(cursor=1, event=event)
```

Bub 没有单独的 event wrapper：每个 event 都是官方 `CloudEvent`，其 `data` 是 ATIF StepObject。`TapeRecord` 只增加某个 committed 位置对应的 store-assigned cursor。identity、type、time、extension 和 ATIF data 都通过 CloudEvents API 读取。
...
插件作者通常只需要 `hookimpl` 与对应 hook 使用的 payload 类型。`TapeSidecar` 是 `provide_tape_sidecar` 返回的公共具名 spec；sibling、overlay 和 committed-event protocol 在不增加更多 hook 的前提下提供对应能力。CloudEvents 与 ATIF 是 tape 原生数据，不是 sidecar。分发和故障隔离语义见 [Hook 参考](/zh-cn/docs/reference/hooks/)。
