...
`BubFramework.process_inbound` 的返回值。`prompt` 是解析后的 prompt(字符串或被强制转为字符串的 list-of-parts);`outbounds` 是所有 `render_outbound` 实现结果的扁平拼接。
...
| `hook_report()` | 返回 hook 名 → 实现该 hook 的 adapter 列表。`bub hooks` 的数据来源。 |
