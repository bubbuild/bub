...
`StreamEvent.kind` 可以是 `text`、`reasoning`、`tool_call`、`tool_result`、`usage`、`error` 或 `final`。`AsyncStreamEvents` 包装异步迭代器，并暴露最终 `error` 与 `usage` 状态。`attach(callback)` 返回共享相同状态的 stream，并在迭代结束时 await callback。
...
插件作者通常只需要 `hookimpl` 与对应 hook 使用的 payload 类型。`TapeSidecar` 是 `provide_tape_sidecar` 返回的最小具名契约；`sidecar_tape_name` 用于解析拥有 sibling tape 的 sidecar 的物理名称。存储型与能力型 sidecar 的语义见 [Hook 参考](/zh-cn/docs/reference/hooks/)。
