...
@dataclass(frozen=True)
class AdmitDecision:
    action: AdmitAction | Literal["process", "drop", "wait", "steer"]
...
`admit_message` 实现返回 `AdmitDecision`,告诉 channel manager 对单条 inbound message 执行 process、drop、wait 或 steer。
...
`SteeringHandle` 会以 `state["_runtime_steering"]` 暴露给 model hooks。`get_nowait()` 取出一条;`drain_nowait()` 取出当前全部 queued messages。返回的消息由 model hook 接管,不会重放。
...
| `steering(session_id)` | 返回暴露给 model hooks 的 per-session steering handle。 |
| `clear_steering(session_id)` | 清除 idle session 的 steering handle。 |
...
| `AdmitAction` | enum | Turn admission action 词汇。 |
| `AdmitDecision` | dataclass | `admit_message` 返回的 decision。 |
| `Settings` | class | 插件配置基类(从 `bub.configure` 重新导出)。 |
| `SteeringHandle` | dataclass | 暴露给 model hooks 的 per-session steering queue handle。 |
