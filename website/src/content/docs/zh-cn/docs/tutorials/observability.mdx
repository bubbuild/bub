...
- 启动带 Phoenix 的 Bub 之前，安装 Logfire extra。它负责配置 Bub 原生 GenAI span 使用的 OpenTelemetry provider 和 OTLP exporter：
...
Bub 把已提交的运行 tape event 通过官方 OpenTelemetry GenAI instrumentation utility 投影为 `invoke_agent`、`chat` 和 `execute_tool` telemetry。Logfire 负责配置 SDK，并通过 OTLP 把结果发送到 Phoenix。内容属性默认关闭；operation lifecycle、model usage、duration 和 error 遵循该 utility 的 OpenTelemetry semantic conventions。
...
:::caution
`OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT=true` 会启用 OpenTelemetry GenAI instrumentation utility 的消息和工具内容采集。除非 telemetry 目的端与数据保留策略已获批准，否则请保持未设置。
:::
