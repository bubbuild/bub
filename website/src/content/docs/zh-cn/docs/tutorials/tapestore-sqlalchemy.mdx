---
title: 使用 SQLAlchemy 将 tape 持久化到 SQLite
...
- Bub 已安装，且 `bub --help` 可以运行（参考 [安装](/zh-cn/docs/getting-started/install/)）。
...
这就是完整的存储切换：Bub 仍然写 append-only tape entries，只是它们现在进入 SQLite 数据库，而不是每条 tape 一个 JSONL 文件。
