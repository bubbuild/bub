...
`bub install` 要求 Bub 自身运行在 virtualenv 中（参见 [`bub install`](/zh-cn/docs/reference/cli/#bub-install)）。独立安装器通过 `uv tool` 为 Bub 创建隔离环境，已经满足该条件。如果从源码 checkout 运行，请改用 `uv run bub install bub-mcp@main`。
