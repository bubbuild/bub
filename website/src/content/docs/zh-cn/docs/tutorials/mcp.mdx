...
`bub install` 要求 Bub 自身运行在 virtualenv 中（参见 [`bub install`](/zh-cn/docs/reference/cli/#bub-install)）。独立安装器会在 `~/.bub/.venv/`（或 `$BUB_HOME/.venv/`）创建该环境。如果从源码 checkout 运行，请改用 `uv run bub install bub-mcp@main`。

安装器的 `recommended` 预设包含 `bub-mcp` 和 `bub-agent-plugins`；如果选择了该预设，两者已经安装。
[Agent Plugins](/zh-cn/docs/tutorials/agent-plugins/) 可通过独立的生命周期 channel 和包含插件名的工具名称，
加载 skills 及其自带的 MCP 服务器，与本教程中的服务器一起使用。
...
如果没有 `mcp`，说明插件被装到了与 `bub` 解析出的环境不同的地方。
...
`bub mcp list` 已经足以确认接入正确。**真正在 turn 中调用工具，需要 Bub 的 channel 运行时**：

- `bub chat` 会启动 `cli` interface，并自动附着已启用的 `Lifecycle` 后台服务，包括 `mcp.lifecycle`。
...
- `bub run` 仍然不会启动 channel，因此不会暴露通过 `mcp.lifecycle` 发现的 MCP 工具。
...
| 症状 | 检查项 |
|---|---|
| `bub hooks` 中没有 `mcp` | 插件没有装进 `bub` 使用的环境。请使用同一个 `bub` 可执行文件重新安装。 |
...
- [加载 Agent Plugins](/zh-cn/docs/tutorials/agent-plugins/) — 在 `mcp.json` 之外加载可移植的 skills 和 MCP 服务器。
