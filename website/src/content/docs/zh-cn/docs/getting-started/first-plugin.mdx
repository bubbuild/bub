...
`build_prompt` 是 `firstresult` hook —— 第一个非 None 的返回值胜出。`run_model_stream` 同样是 `firstresult`，所以本实现完全覆写了内置 agent。即使 CLI 最后缓冲后再展示文本，模型执行也始终使用这一 stream 契约。
...
你应当在 `build_prompt` 与 `run_model_stream` hook 项下看到 `echo` 与 `builtin` 并列。
