...
The builtin spill plugin mounts a `SpillStore` through `provide_tape_sidecar`, registers `spill.read`, and bounds large results through the existing `after_tool_call` hook. This includes failed tool results after their structured errors are rendered as JSON; the execution still remains a failure. The sidecar itself has no tool-result interception contract. Large results are stored in a sibling tape named `<session-tape>__sidecar__spill`. The sidecar uses the same `TapeStore` as the session tape, so existing storage plugins do not need a spill-specific interface. Bub writes UTF-8-safe chunks followed by a manifest; the manifest is the completion marker for the stored result.
