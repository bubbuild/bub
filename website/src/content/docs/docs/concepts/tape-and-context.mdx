...
The builtin spill plugin mounts a `SpillStore` through `provide_tape_sidecars`. Large string tool results are stored in a sibling tape named `<session-tape>__spill`. The sidecar uses the same `TapeStore` as the session tape, so existing storage plugins do not need a spill-specific interface. Bub writes UTF-8-safe chunks followed by a manifest; the manifest is the completion marker for the stored result.
...
Lifecycle outcomes are recorded on the main tape as `spill.write`, `spill.archive`, `spill.reset`, and `sidecar.merge` events. These entries are marked as context-excluded before any context selector runs, so they remain available for operations and audit without changing model messages or prompt-cache prefixes. A sidecar persistence failure is recorded but does not prevent the main tape from merging or resetting.
