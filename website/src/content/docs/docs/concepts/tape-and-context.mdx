...
Entries marked `context=False` are removed before the selector runs. This is how operational spill events remain queryable on the tape without appearing in either the default context or a plugin-defined context.

### Session and turn isolation

Each session id addresses an ordinary session tape. An inherited subagent resolves the parent's session id, a temporary subagent uses a throwaway session id, and a named subagent session addresses its own tape.

The builtin `forkmerge` sidecar isolates one turn with an in-memory tape overlay. Its caller merges persistent runs or discards temporary runs. The sidecar does not create logical sessions or persistent fork tapes.
