...
- `tool_result` → one `tool` role message per result, joined back to the matching `tool_call_id`.
...
3. Retries the same prompt — the new anchor truncates history, so the rebuilt context fits.

This is one retry per turn; if the second attempt still overflows, the error is raised normally.
