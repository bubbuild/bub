...
Large string tool results are stored in a sibling tape named `<session-tape>__spill`. The sidecar uses the same `TapeStore` as the session tape, so existing storage plugins do not need a spill-specific interface. Bub writes UTF-8-safe chunks followed by a manifest; the manifest is the completion marker for the stored result.
...
The sidecar follows the session tape through forks, merges, archive, and reset, but remains a separate tape and is never scanned while constructing the main context. It can also be archived or reset independently with `Tape.archive_spill()` and `Tape.reset_spill()`. When reset requests an archive, Bub preserves the sidecar if that archive fails.
