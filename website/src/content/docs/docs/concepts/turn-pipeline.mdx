...
flowchart TD
  ...
  prompt --> model["run_model_stream<br/>firstresult"]
...
`run_model_stream` is a `firstresult` hook and the only model-execution path. It returns an `AsyncStreamEvents` iterator of chunks (`text`, `error`, ...). The kernel always consumes that stream and joins text deltas into the final model output. `stream_output` controls only whether the channel router presents chunks while they arrive; it does not select another execution API.

If no implementation returns a stream, the kernel notifies `on_error(stage="run_model_stream", ...)` and falls back so `save_state` and the outbound stages still run. A string prompt is returned unchanged; a multimodal list prompt falls back to the inbound envelope's plain `content`.

While streaming, every `error` event is forwarded through `on_error(stage="run_model_stream", ...)`. A bound `ChannelRouter` may also wrap the stream to push partial chunks to the channel before the turn finishes.

### save_state

Broadcast, **always invoked** in a `finally` block — even if `run_model_stream` raises. Implementations receive `session_id`, `state`, the inbound `message`, and the captured `model_output` (which may be empty if the model raised).
...
Broadcast, called per outbound envelope. Implementations may return `True` once the envelope is delivered. The built-in implementation also forwards the envelope to a bound `ChannelRouter` for actual channel I/O.
...
- `run_model_stream` — delegates to `Agent.run_stream`, the [Republic](https://github.com/bubbuild/republic)-backed loop with tool use and auto-handoff (see [Tape and context](/docs/concepts/tape-and-context/)).
...
- `dispatch_outbound` — forwards through the bound `ChannelRouter`.
...
- `provide_tape_sidecar` — builtin sibling, overlay, and committed-event specs.
