...
While streaming, every `error` event is forwarded through `on_error(stage="run_model", ...)`. A bound `ChannelRouter` may also wrap the stream to push partial chunks to the channel before the turn finishes.
...
- `provide_tape_sidecar` — the builtin `spill` and `forkmerge` sidecars over the configured plain tape store.
