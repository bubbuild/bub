...
```mermaid
flowchart LR
  channels["Channels<br/>outward I/O"] --> envelope_io["Envelope<br/>in and out of the turn pipeline"]
  skills["Skills<br/>operator procedures"] --> envelope_prompt["Envelope<br/>loaded into the prompt as text"]
  tools["Tools<br/>model-callable actions"] --> state["State<br/>per-turn dict with runtime keys"]
```
