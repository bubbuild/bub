...
flowchart LR
  ...
  tools["Tools<br/>model-callable actions"] --> state["TurnState<br/>(per-turn dict<br/>with _runtime_*)"]
```

All three surfaces meet on two small kernel contracts:
...
- **`TurnState`** — a per-turn dict from [`bub.turn`](https://github.com/bubbuild/bub/blob/main/src/bub/turn.py) that hooks contribute to via [`load_state`](/docs/concepts/turn-pipeline/), and that survives until [`save_state`](/docs/concepts/turn-pipeline/) finishes.
...
- [Core contracts reference](/docs/reference/types/) — subsystem-owned types and accessor helpers.
