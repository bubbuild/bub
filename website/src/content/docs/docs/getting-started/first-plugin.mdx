...
class EchoPlugin:
    ...
    def run_model_stream(self, prompt, session_id, state):
        ...
        async def events():
            yield StreamEvent("text", {"delta": f"[echo:{session_id}] {text}"})
...
`build_prompt` is a `firstresult` hook — the first non-None return value wins. `run_model_stream` is also `firstresult`, so this implementation overrides the built-in agent entirely. Model execution always uses this stream contract, even when the CLI buffers the text for final display.
...
You should see `echo` listed under the `build_prompt` and `run_model_stream` hook entries alongside `builtin`.
