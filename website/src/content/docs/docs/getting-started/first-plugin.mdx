...
## 3. Install it editable

From the plugin directory, install the package into your active environment:

```bash
uv pip install -e .
```

Bub loads entry points from the active Python environment, so an editable install is enough for iteration.
...
If the response came from a real model instead, the plugin did not load — re-run `uv pip install -e .` in the active environment and re-check `bub hooks`.
