...
The installer places `uv` and the `bub` executable in user-level directories. It also asks `uv` to add its executable directory to your shell profile. Restart your shell after installation, then run `bub --help`.

You can inspect the scripts before running them at [install.sh](https://bub.build/install.sh) and [install.ps1](https://bub.build/install.ps1).
