...
- [Bub installed](/docs/getting-started/install/) and `bub hooks` reporting the `builtin` plugin
- one model access path: an API key exported as `BUB_API_KEY` or a provider-specific `BUB_<PROVIDER>_API_KEY`; alternatively, OpenAI Codex OAuth via `bub login openai` with `BUB_MODEL=openai:<codex-model>`
...
```bash
bub run "Summarize the purpose of this workspace."
```
...
Bub stores the credential locally. Use it by setting `BUB_MODEL=openai:<codex-model>` for a model available to your Codex account, leaving `BUB_API_KEY` unset, and not setting a custom `BUB_API_BASE`.
