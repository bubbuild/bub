...
Bub loads entry points from the active Python environment, so editable installs pick up code changes without reinstalling. The same pattern shown in [First plugin](/docs/getting-started/first-plugin/) applies here.
...
Each registered hook lists the plugins that contribute to it. Expected output for an `EchoPlugin` that overrides `build_prompt` and `run_model`:
