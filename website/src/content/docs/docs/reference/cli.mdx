...
Install a plugin into Bub's managed `uv` project (`~/.bub/bub-project` by default), or sync the project when no specs are passed.
...
| Option | Type | Default | Description |
| --- | --- | --- | --- |
| `SPECS` (args) | one or more strings | empty list | Package specs: PyPI name, `owner/repo[@ref]`, `git+...`, or bare bub-contrib package name resolved against `https://github.com/bubbuild/bub-contrib.git`. |
| `--project` | PATH | `~/.bub/bub-project` (env: `BUB_PROJECT`) | Path to the Bub plugin project directory. |
...
| `--project` | PATH | `~/.bub/bub-project` (env: `BUB_PROJECT`) | Plugin project directory. |
...
The command is registered with `hidden=True`, so it does not appear in the top-level `--help` listing. Output is one line per hook: `hook_name: adapter1, adapter2, …`. Used to verify which plugins win each hook (later registration wins; see [Hooks › How hooks are invoked](/docs/reference/hooks/#how-hooks-are-invoked)).
