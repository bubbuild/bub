...
| `build_prompt` | firstresult | `(message: Envelope, session_id: str, state: State) -> str \| list[dict]` | prompt or content parts | `BubFramework.process_inbound` | Falls back to `content_of(message)` if all impls return `None`. |
...
| `save_state` | broadcast | `(session_id, state, message, model_output) -> None` | none | `process_inbound` finally block | Always runs, even on turn failure. |
...
| `provide_channels` | sync-only consumer (deduped) | `(message_handler: MessageHandler) -> list[Channel]` | channels | `BubFramework.get_channels` (`call_many_sync`) | Channels are deduplicated by `Channel.name`; first registration wins. |
...
:::caution
Defining an `async def` impl for any sync-only hook silently disables it. Confirm registration with `bub hooks`.
:::
