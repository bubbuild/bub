...
| `provide_tape_sidecars` | sync-only consumer (deduped) | `() -> TapeSidecar` | mounted sidecars | `BubFramework.get_tape_sidecars` | Each implementation contributes one sidecar. Sidecars are deduplicated by `name`; the first value in hook priority order wins. |
...
Bootstrap hooks **must** be synchronous: `register_cli_commands`, `onboard_config`, `provide_channels`, `provide_tape_store`, `provide_tape_sidecars`, `build_tape_context`, plus `system_prompt`.
