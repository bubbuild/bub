...
Each `provide_tape_sidecar` implementation contributes one named capability. Storage sidecars own a sibling tape in the active `TapeStore`; for example, a scoped session maps `spill` to `<session-tape>__sidecar__spill`, and the main tape includes it in merge, archive, and reset operations. A capability-only sidecar sets `owns_tape = False` and manages its own lifecycle. The builtin `forkmerge` sidecar uses that form to isolate writes over a plain session tape; its caller chooses `merge()` or `discard()`. Removing a provider stops mounting its sidecar but does not delete stored data.
