...
3. **`~/.bub/config.yml`** entry, loaded by `bub.configure.load`.
4. **Field default** declared on the `Settings` subclass.
...
`~/.bub/` is also the value of `bub.home`, controlled by the `BUB_HOME` environment variable (`src/bub/__init__.py`).

## Framework — runtime paths

| Env var | Default | YAML key | Read by | Description |
| --- | --- | --- | --- | --- |
| `BUB_HOME` | `~/.bub` | — | `bub.home` (`bub/__init__.py`) | Root directory for config, tape store, and the managed plugin project. |
| `BUB_PROJECT` | `~/.bub/bub-project` | — | `--project` option in `bub install` / `uninstall` / `update` | Plugin project directory; created on first `install` via `uv init`. |
