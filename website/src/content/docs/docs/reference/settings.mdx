...
class AgentSettings(Settings):
    ...
    tool_spill_threshold: int = Field(default=4096, ge=0)
...
| `BUB_TOOL_SPILL_THRESHOLD` | `4096` | `tool_spill_threshold` | Estimated tokens (4 chars each) above which string tool results are stored in a chunked spill tape. Set to `0` to disable. |
