...
The per-turn state dict. The framework seeds it with `_runtime_workspace`, then merges the results of every `load_state` hook before the model call. The same dict is passed to `build_prompt`, `run_model[_stream]`, `save_state`, `render_outbound`, and `system_prompt`.
...
These dataclasses live in `src/bub/runtime_options.py` and are exported from `bub`. They describe protocol-neutral runtime choices that a channel or adapter may present to a user.
...
`id` is the stable value passed back through the channel or adapter when a turn runs. Bub does not own UI state for these choices; callers store selections and pass them on inbound messages as runtime metadata.
...
`admit_message` implementations return `AdmitDecision` to tell the channel manager whether to process immediately, drop, queue as follow-up input, or route as steering input. The `steer` handler is currently a placeholder and falls back to the follow-up queue until dedicated steering handling is rebuilt.
...
| `get_runtime_options(...)` | Collect protocol-neutral runtime choices from hooks, appending model choices in hook priority order. |
...
| `RuntimeChoice` | dataclass | One selectable runtime value. |
| `RuntimeOptions` | dataclass | Available runtime choices for models. |
