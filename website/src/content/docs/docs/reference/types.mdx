...
description: Bub's subsystem-owned contracts for turns, models, tape, streaming, hooks, and channels.
---
...
### Tape events and records — `bub.tape`
...
Bub has no separate event wrapper: every event is an official `CloudEvent`, and its `data` is an ATIF StepObject. `TapeRecord` adds only the store-assigned cursor for one committed position. Use the CloudEvents API to read identity, type, time, extensions, and ATIF data.
...
Plugin authors normally need only `hookimpl` plus the payload types used by their hooks. `TapeSidecar` is the common named spec returned by `provide_tape_sidecar`; the sibling, overlay, and committed-event protocols add capability-specific behavior without adding more hooks. CloudEvents and ATIF are native tape data, not sidecars. See the [Hook reference](/docs/reference/hooks/) for dispatch and fault-isolation semantics.
