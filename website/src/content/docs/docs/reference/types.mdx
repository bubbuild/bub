...
`StreamEvent.kind` is one of `text`, `reasoning`, `tool_call`, `tool_result`, `usage`, `error`, or `final`. `AsyncStreamEvents` wraps an async iterator and exposes terminal `error` and `usage` state. `attach(callback)` returns a stream that shares the same state and awaits the callback when iteration ends.
...
Plugin authors normally need only `hookimpl` plus the payload types used by their hooks. `TapeSidecar` is the minimal named contract returned by `provide_tape_sidecar`; `sidecar_tape_name` resolves the physical name for a sidecar that owns a sibling tape. See the [Hook reference](/docs/reference/hooks/) for storage and capability-only sidecar semantics.
