...
Reads `message[key]` if `message` is a `Mapping`, else `getattr(message, key, default)`.
...
Returned by `BubFramework.process_inbound`. `prompt` is the resolved prompt (string or list-of-parts coerced to string); `outbounds` is the flattened result of every `render_outbound` impl.
...
| `hook_report()` | Map hook name → ordered list of adapter names. Backs `bub hooks`. |
