...
`bub install` requires Bub itself to run from a virtual environment (see [`bub install`](/docs/reference/cli/#bub-install)). The standalone installer already satisfies this because `uv tool` gives Bub an isolated environment. When running from a source checkout, invoke the command with `uv run bub install bub-mcp@main` instead.
