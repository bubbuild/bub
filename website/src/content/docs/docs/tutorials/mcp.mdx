...
`bub install` requires Bub itself to run from a virtual environment (see [`bub install`](/docs/reference/cli/#bub-install)). The standalone installer already creates one at `~/.bub/.venv/` (or `$BUB_HOME/.venv/`). When running from a source checkout, invoke the command with `uv run bub install bub-mcp@main` instead.

The installer's `recommended` preset includes `bub-mcp` and `bub-agent-plugins`. If you selected it, both are
already installed. [Agent Plugins](/docs/tutorials/agent-plugins/) can load skills and their own MCP servers
alongside the servers in this tutorial, using a separate lifecycle channel and plugin-qualified tool names.
...
If `mcp` is missing, the plugin landed outside the environment used by `bub`.
...
```bash
bub gateway --enable-channel cli --enable-channel mcp.lifecycle
```
...
bub mcp add --transport stdio --env API_KEY=secret example -- node ./my-server.js

# http / sse
bub mcp add --transport http weather https://weather.example.com/mcp
bub mcp add --transport sse --header "Authorization: Bearer token" \
...
| Symptom | Check |
|---|---|
| `mcp` does not appear in `bub hooks` | The plugin was installed outside the environment used by `bub`. Re-install it with the same `bub` executable. |
...
- [Load Agent Plugins](/docs/tutorials/agent-plugins/) — load portable skills and MCP servers alongside `mcp.json`.
