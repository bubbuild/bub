...
- Bub installed with the standalone installer or into an active virtual environment, with `bub --help` working. Both paths satisfy the virtual-environment requirement of `bub install`.
...
```bash
bub run "Say hello from a prompt built outside Python."
```
