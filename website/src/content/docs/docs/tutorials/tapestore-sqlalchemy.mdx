---
title: Persist tapes in SQLite with SQLAlchemy
...
- Bub installed and runnable with `bub --help` (see [Install](/docs/getting-started/install/)).
- A workspace where `bub run "What tools do you have?"` can call your configured model.
...
```bash
bub run "Reply with one short sentence: hello from local SQLite."
bub run ",tape.info"
```
...
That is the full storage switch: Bub still records append-only tape entries, but they now live in a SQLite database instead of per-tape JSONL files.
