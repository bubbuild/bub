...
For a new Bub installation, choose the `lody` preset so the ACP server is part of the initial environment:

```bash
python3 install-bub.py --preset lody --no-interactive
```

If Bub is already installed, add `bub-acp-server` to the same environment that provides your `bub` command:

```bash
bub install bub-acp-server
```
...
Use this wrapper only when you need it. If `bub run` already works from the editor's environment, you can launch `bub acp` directly.
...
| Bub starts but reports a configuration error | Confirm `bub run` works in the same environment. If required settings are in `.env`, load that file before `bub acp`. |
| The editor cannot load or resume a thread | Confirm the editor is launching the same `bub` environment where `bub acp --help` works, and keep `BUB_HOME` stable across launches. |
