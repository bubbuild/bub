...
```bash
bub gateway --enable-channel telegram
```

Enable several:

```bash
bub gateway --enable-channel telegram --enable-channel wechat
```

Exclude a default lifecycle channel:

```bash
bub gateway --enable-channel all --enable-channel '!mcp.lifecycle'
```
