...
Bub reads configuration from three places, in order of precedence (highest first):

1. Process environment (`BUB_*` variables, plus a few provider variables such as `OPENROUTER_API_KEY`).
...
`bub onboard` walks through the minimum config — model, API key, optional channel credentials — and writes the result to `~/.bub/config.yml`.

```bash
uv run bub onboard
```

The file lives at `~/.bub/config.yml` by default. Set `BUB_HOME` to relocate the whole `~/.bub/` directory (config, history, plugin project).
...
| `BUB_HOME` | Bub's data directory (config, history, plugin project). Default `~/.bub`. |
...
Print the loaded hooks (this also forces config validation):
...
Expected output is one line per outbound, prefixed with `[<channel>:<chat_id>]`.
