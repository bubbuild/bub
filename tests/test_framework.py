...
def test_create_cli_app_sets_workspace_and_context(tmp_path: Path) -> None:
    ...
    framework.plugin_manager.register(CliPlugin(), name="cli-plugin")
...
def test_collect_onboard_config_passes_accumulated_updates_to_later_hooks(write_config) -> None:
    with patch.dict(os.environ, {}, clear=True):
        ...
        framework.plugin_manager.register(FirstPlugin(), name="first")
        framework.plugin_manager.register(SecondPlugin(), name="second")
...
async def test_framework_admit_message_calls_hook_with_snapshot() -> None:
    ...
    framework.plugin_manager.register(AdmissionPlugin(), name="admission")
...
async def test_process_inbound_streams_when_requested() -> None:  # noqa: C901
    ...
    framework.plugin_manager.register(StreamingPlugin(), name="streaming")
